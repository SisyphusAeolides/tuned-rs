...
pub fn apply_battery_options(rollback: &Rollback, options: &[(String, String)]) -> Result<()> {
    ...
    if updated > 0 {
        info!("Applied {updated} battery tuning option(s)");
    }
    Ok(())
}
...
fn apply_battery_option(rollback: &Rollback, key: &str, value: &str) -> Result<bool> {
    match key {
        "charge_start_threshold" => {
            apply_charge_threshold(rollback, "charge_control_start_threshold", value)
        }
        "charge_stop_threshold" => {
            apply_charge_threshold(rollback, "charge_control_end_threshold", value)
        }
        "conservation_mode" => apply_conservation_mode(rollback, value),
        "battery_care_limit" => apply_battery_care_limit(rollback, value),
        _ => {
            warn!("Unknown battery option: {key}");
            Ok(false)
        }
    }
}
...
fn apply_charge_threshold(rollback: &Rollback, threshold_name: &str, value: &str) -> Result<bool> {
    ...
    for entry in fs::read_dir(battery_path)? {
        ...
        rollback.record_original(
            &rollback_key("sysfs", &threshold_path.to_string_lossy()),
            &original,
        )?;
...
fn apply_conservation_mode(rollback: &Rollback, value: &str) -> Result<bool> {
    let conservation_path =
        Path::new("/sys/bus/platform/drivers/ideapad_acpi/VPC2004:00/conservation_mode");
    ...
    let mode_value = match value {
        ...
        "off" | "0" | "false" => "0",
        _ => {
            warn!("Invalid conservation mode value: {value}");
            return Ok(false);
        }
    };

    let original = read_trimmed(conservation_path)?;
    if original == mode_value {
        return Ok(false);
    }

    rollback.record_original(
        &rollback_key("sysfs", &conservation_path.to_string_lossy()),
        &original,
    )?;
...
fn apply_battery_care_limit(rollback: &Rollback, value: &str) -> Result<bool> {
    ...
    rollback.record_original(
        &rollback_key("sysfs", &care_limit_path.to_string_lossy()),
        &original,
    )?;
...
pub fn get_battery_status() -> Result<BatteryStatus> {
    ...
    let capacity = read_trimmed(&battery_path.join("capacity"))?
        .parse()
        .unwrap_or(0);
    let status = read_trimmed(&battery_path.join("status"))?;
    let health =
        read_trimmed(&battery_path.join("health")).unwrap_or_else(|_| "Unknown".to_string());
